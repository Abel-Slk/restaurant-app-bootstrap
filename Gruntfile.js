...
module.exports = function(grunt) {
    ...
    //Grunt works on configuration, so every Grunt plugin that we wish to employ for performing a task needs to be configured inside this grunt.initConfig file
        ...
        sass: { // the sass task
            dist: {
                files: {
                    'css/styles.css': 'css/styles.scss' // the syntax here is "destination : source"! (note that this Grunt syntax is reversed compared to the sass terminal script - even though we're doing the same!)  
                }
            }
        },
        watch: { // the watch task: keep a watch on the .scss files and then automatically compile them by invoking the sass task
            ...
            //OR - mb it IS correct - if the files in the watch task OVERRIDE the files in sass?? and does it automatically replace the two occurrences of css/styles with css/* in the sass task's files??
        },
        browserSync: {
            dev: {
        ...
        copy: { // the copy task
            html: { // copying html files is needed for usemin. usemin expects us to also copy our HTML files to the distribution folder so that it can do the manipulation on that. (this html property corresponds to what we had inside the usemin script in package.json when we were using npm scripts)
        ...
        useminPrepare: { // usemin: The first task that I need to configure is called useminPrepare. This useminPrepare task will prepare the files and then also configure the concat, cssmin, uglify and filerev plugins, so that they can do their work correctly. So, that's the reason for the useminPrepare. this is how the grunt usemin plugin has been designed. Somewhat different from the way the usemin that we used with the npm scripts works. 
        ...
        // Now how does this filerev can compute this value? It takes the contents of these files and then does some processing and then generates an md5 compressed 20 characters number which will be attached to the main file.
            ...
            release: { // filerev:release hashes (md5) all assets (images, js and css) in the dist directory
                files: [{
                    src: [
                        'dist/js/*.js',
                        'dist/css/*.css',
    ...
    grunt.registerTask('build', [ // build will do 3 tasks  - in the following order:
        ...
        'concat',
        'cssmin',
        'uglify', // after this the main.css" and main.js files are ready, and I can do the "filerev" on them and then finally run the "usemin:
        'filerev',
        'usemin'
    ]);
};